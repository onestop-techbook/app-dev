...
そこで、本書では、少々規模は大きくなるものの、ある程度実用性のある「データ解析ツール」なるものを作ってみることにする。本業が、大規模プラントにおける計測・制御の研究開発をやっている関係で、多数のデータを取って、そのデータを処理していくというのはよく直面する。ところが当然ながらデータは案件ごとに異なる。当然データの構造も違えば、行いたい解析は異なる。データの解析をいちいちエクセル等で実行するのは非常に面倒だし、マクロを作るのも難しいので、解析待ちの積みデータが増える。

データ解析がかったるいと、積みデータが増え、せっかく取ったデータが腐るし、開発スピードも低下する。

そこで、自分が必要なデータを読み込み、前処理し、必要な演算を行い、保存用データとして出力するとともに、グラフ化、画像化するツールを作る。

ここで扱う機能は、ファイルの入出力、四則演算、画像生成、マウスによる操作・値抽出など、プログラミングの基礎的な内容はだいたい盛り込まれているし、自分で作るツールであるから、自分が必要な機能を簡単に実装する事ができる。

また、一度ベースとなるソフトを作っておけば、似たような案件、次の別の案件においても小改造で活用することができ、次案件の着手速度がUPする。また、グループでの共有も可能になり、グループにおける秘伝のタレとなることも期待できる。実際自分の所属するグループでそうなっているコードもあるのだが、例えば開発環境がレガシーだったり、元の作者の癖があったりで読みづらいなど、すぐに使うのは少々しんどい状況にあったりするので、自分の秘伝のタレを作っておくことが望ましい。
...
ここでの工夫としては、「取れるものは全部取っておく」が基本である。もちろんサンプリング間隔やハードの点数の問題もあるが、例えば、測定時刻、生データのみならず、測定回数のインクリメント、測定開始からの経過日数等も準備しておくと、このあとのコーディングが楽になることが少なくない。
...
ハードコーディングとは、計算に必要な数値(係数など)を直接ソースコードに書くことである。例えば、商品単価から支払額を算出する場合に、消費税(現在8％)を、ソースコード中に*0.08と記入するようなものである。
...
随所からマサカリが飛んでくる可能性は十分承知しているが、ハードコーディングする理由を述べる。例えば係数を外部ファイルを参照するなどすると、その定義ファイルを書き換えるだけで修正できるので、便利であることは明らかである。しかし、極論すれば、×2を計算するために、x2という係数を記録した設定ファイルを読み込む必要があるなど、必要な手数が大きく増加する。プログラミング初心者にとって、「直ちに意図がつかめないおまじない」の存在は、つまづきの原因となる。しかも、このハードコーディングを避けるためのおまじないは、自分で意味がつかめる数値のハードコーディングとは異なり、ファイルを読み込むためのよくわからないコードや中身のわからない変数列となり、全コードの網羅理解を妨げる。

例えば、ハードコーディングすればA=a*2;というコードで済むところが、係数を変数で定義し、係数を記録したファイルを定義し、ファイルを読み込み、係数を探し、係数を変数に代入し、ようやくA=a*2が得られる。という例を考えてほしい。コード量が増え、理解するのに時間がかかる。係数が頻繁に変わったり、個数が多いなどの場合は当然それに見合った対応が必要になるが、ハードコーディングすればシンプルで済むという意図を理解してもらえると思う。
...
もともとの波形を知っていれば、50～75付近に信号があることがわかるが、非常に大きなノイズに埋もれていて、ほとんど見分けがつかない。
...
命名規則は個人によりけりだと思うが、コントロールの種類（ボタンなのか、テキストボックスなのか、ラベルなのかチェックボックスなのかなど）がひと目で分かり、かつそれが何をするものなのか。（ボタンであれば、ファイルを開くボタンなのか、終了するボタンなのか、など）がわかるように、名前をつけることが望ましい。今回のボタンは、csvファイルをひらくボタンであるから、btbOpenCSVと名前をつけた。なお、筆者は、できるだけ以下のように名前を付けるようにしている。
...
一人開発ならばあまり細かいことは気にしなくても良いが、グループ開発ならば、Gitのお作法についてはぜひ注意されたい。
...
Double a = -8;  Double b = 3;　Single c = a / b;　//NG
Double a = -8;　Int16 b = 3;　 int c = (int)(a / b);　//OKだが、丸められる。
//}

このように、倍精度浮動小数点=Double>単精度浮動小数点=Single (Float) > 整数intと、広いからである。同様に、整数型の中でもInt(32bit)とShort(16bit)などで同じ現象が起こる。

このことは、プログラミングになれた人にとっては当たり前の内容であるが、初めての人には案外躓く内容である。実際この本を書くにあたって実装している途中で型で苦しめられるのは何回もあったからである。また、筆者はVBから入った人なので、その差にも戸惑った。VBは型の制約が緩いためである。
