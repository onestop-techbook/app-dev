...
Pythonの開発環境構築の章でも言及しましたが、Node.jsも同様にバージョン管理ができるように設定することが重要です。
...
ただし、注意点としてWindowsには対応しておらずLinuxかMac OSのみの対応となります。
...
Node.jsの開発にもVSCodeの利用が便利です。
上述の「ESLint」、「prettier」もVSCodeに設定することができるため、
VSCode内でコードの整形や警告の表示を行う事ができます。
また、デバッグ実行を行うことも可能です。
特にDebugger for Chromeという拡張機能を使うと、VSCodeでブラウザのJavScriptやTypeScriptのでバッグを行うことが可能です。
Webアプリケーション開発において、ブラウザ上でのJavaScriptのでバッグは非常に重要ですので、おすすめです。
