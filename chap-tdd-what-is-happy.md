...
「テスト駆動開発」の概念そのものは他の章（例えば、章「自動テスティング」の節「ユニットテスト」や節「TDD」）に説明を譲るとして、
...
1. 「A+B*2を返して欲しい」をテストコードとして表現する◆式へのリンク◆
2. A+B*2を実装する
...
describe('name_and_info.js', () => {
    describe('extractName()', ()=>{
        it('returns just name.', ()=>{
            ...
            var KEY2 = '07johndoe__hogahogepiyo';
...
注釈：テスト結果が分かり易いのは、テストフレームワークや検証ライブラリの機能のおかげです。
　　　したがって、その辺りが整備されていない言語や、利用できない環境の場合は、テスト駆動開発の快適さと安心さの程度は下がります。
...
実際にざっと書いてみると分かりますが、上記◆リンクする◆のコード中の
...
上記をテストコードで表現すると、次◆式へリンク◆のようになります。
...
上記の式◆リンクする◆を書く中で、疑問が湧いてきて書き直したように、関数の実装前にテストコードを書くことで、
「何を前提として、どういう機能を作ろうとしているのか？」を明確にすることが出来ます。
「何を前提として」がテストコードから明確に分かりますので、いわゆる「部品の再利用」をし易い
設計にもできるでしょう。

本節のテストコードを実行すると、その関数の実装具合によって、次のように
実行結果は変わります。そのあたりの詳しい説明はNode.js出の実装説明となるため、
本書の目的の範囲を超えるの割愛します。
